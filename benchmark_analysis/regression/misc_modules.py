...
def plot_double_standard(
    ...
    sent_color=setting_dict['sent_color']
    no_sent_color= setting_dict['no_sent_color']
    font_size= setting_dict['font_size']
    figsize=  setting_dict['figsize']
    fontfamily= setting_dict['fontfamily']
    save= setting_dict['save'] 
    ...
    plt.savefig(save_at, bbox_inches='tight') if save else None
    plt.show()  

def single_plot( date,actual,predictions , y_label = 'log_return') : 
    time = np.asarray(pd.to_datetime(date))
    ...
    # Plotting the graph
    plt.plot(time, np.asarray(actual).reshape(-1,1),'k--', label='y_actual',linewidth=1, dashes=(6, 5))  # Dotted red line for y_actual
    plt.plot(time, np.asarray(predictions).reshape(-1,1), 'b', label='ymean',linewidth=1)  # Blue line for ymean
